-- ============================================================================
--  LearnHub 种子数据（由 scripts/make-seed-sql.js 生成，请勿手改）
--  演示账号密码统一为 demo1234
--  应用： npx wrangler d1 execute learnhub --local  --file=./migrations/0002_seed.sql
--         npx wrangler d1 execute learnhub --remote --file=./migrations/0002_seed.sql
-- ============================================================================

-- 幂等：已存在则跳过
INSERT OR IGNORE INTO users (id,email,name,role,is_admin,password_hash,school,student_no,is_active,created_at) VALUES
...
INSERT OR IGNORE INTO courses (id,title,description,cover_emoji,join_code,teacher_id,is_published,created_at) VALUES
  (1,'多传感器信息融合滤波技术','卡尔曼滤波 / EKF / UKF 与多传感器加权融合，含状态估计方差对比实验。','🛰️','DEMO01',2,1,'2026-09-19 14:51:39');

INSERT OR IGNORE INTO enrollments (course_id,student_id,created_at) VALUES (1,3,'2026-09-19 14:51:39');

-- 知识点
INSERT OR IGNORE INTO knowledge_points (id,course_id,title,content,order_no,scope,created_by,created_at) VALUES
...
-- 公告
INSERT OR IGNORE INTO announcements (id,course_id,content,created_at) VALUES
  (1,1,'本周重点：UKF 的无迹变换与 Sigma 点选取，请完成课后作业并做一遍知识点自测。','2026-09-19 14:51:39');

-- 作业
INSERT OR IGNORE INTO assignments (id,course_id,title,content,due_at,full_score,created_at) VALUES
...
-- 试题
INSERT OR IGNORE INTO quiz_sets (id,course_id,title,source,scope,kp_ids,created_by,is_published,created_at) VALUES
  (1,1,'第 1 章 · 卡尔曼滤波基础自测','manual','course','[1]',2,1,'2026-09-19 14:51:39');

INSERT OR IGNORE INTO questions (id,quiz_set_id,qtype,stem,options,answer,analysis,difficulty,kp_id,order_no) VALUES
