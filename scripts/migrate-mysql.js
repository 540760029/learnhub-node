 ...
 *
 * 读取 migrations/mysql/0001_schema.sql（由 npm run sql:mysql 从 SQLite schema
 * 自动翻译而来）并逐条执行。
 *
 ...
 *
 * 环境变量：LEARNHUB_DB_URL=mysql://user:pass@host:3306/db
 */
...
const url = process.env.LEARNHUB_DB_URL
  || (fs.existsSync(path.join(ROOT, '.dev.vars'))
    ? (fs.readFileSync(path.join(ROOT, '.dev.vars'), 'utf8').match(/LEARNHUB_DB_URL=(.+)/) || [])[1]
    : null);
...
if (!fs.existsSync(file)) {
  console.error('❌ 找不到 migrations/mysql/0001_schema.sql，请先运行 npm run sql:mysql');
  process.exit(1);
}
