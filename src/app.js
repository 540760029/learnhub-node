/**
 * LearnHub API —— Fetch 原生实现，Cloudflare Workers 与 Node 通用
 *
 * 为什么不用 Express：Express 依赖 on-finished 读 req.socket，Workers 的 Request 没有
 * socket，任何 Express 应用在 Workers 上都会 500（已实测）。这里改用 Fetch 标准 API：
 *   - Workers 原生运行，Node 18+ / Deno / Bun 同样可跑
 *   - 零运行时依赖（package.json 里没有 dependencies）
 *
 * 与 Python 版接口完全兼容，前端 public/ 目录直接复用。
 */
...
export function createApp({ db, env, readStatic = null }) {
  ...
  app.post('/api/courses/:cid/quizzes', async (c) => {
    ...
    await db.batch(prepared.map((q, i) => db.d1.prepare(
  ...
  app.post('/api/ai/generate', async (c) => {
    ...
    await db.batch(questions.map((q, i) => db.d1.prepare(
      `INSERT INTO questions (quiz_set_id, qtype, stem, options, answer, analysis, difficulty, kp_id, order_no)
       VALUES (?,?,?,?,?,?,?,?,?)`,
  ...
  // ================================================================ 静态资源
  // 生产：Cloudflare Workers Static Assets（ASSETS 绑定，public/ 不打包进 Worker）
  // 本地：由 server.js 传入 readStatic(path) 从文件系统读
  //
  // 注意：这里刻意不 import node:fs —— 应用层必须保持运行时可移植，
  // 否则在 Workers 里会直接崩。文件系统能力由运行时通过 readStatic 注入。
  app.use(async (c) => {
    if (c.method !== 'GET' && c.method !== 'HEAD') return undefined;
    if (c.path.startsWith('/api/')) return undefined;

    const assets = env && env[cfg.assetsBinding];
    if (assets && typeof assets.fetch === 'function') {
      const resp = await assets.fetch(new Request(c.req.url, { method: 'GET' }));
      return new Response(resp.body, { status: resp.status, headers: resp.headers });
    }

    if (typeof readStatic === 'function') {
      const rel = c.path === '/' ? '/index.html' : c.path;
      const hit = await readStatic(rel);
      if (hit) return hit;
      const fallback = await readStatic('/index.html');    // SPA 回退
