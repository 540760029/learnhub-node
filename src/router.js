 ...
 * 极简 Fetch 原生路由器
 *
 * 为什么不用 Express：Express 依赖 on-finished 读取 req.socket，
 * 而 Cloudflare Workers 用的是 fetch 风格 Request（没有 socket），
 * 任何 Express 应用在 Workers 上都会 500（已实测）。
 *
 * 这个路由器只用 Request/Response 标准 API，因此：
 *   - Cloudflare Workers 原生运行
 *   - Node 18+ / Deno / Bun 也能直接跑（零依赖，行为一致）
 *
 * 用法：
 *   const r = new Router();
