/**
 * LearnHub (Node + D1) 端到端测试
 *
 *   node --test test/
 *
 * 用 node:sqlite 适配器把 D1 接口在本地跑起来，覆盖：
 *   认证 / 多教师隔离 / 知识点可见性 / 作业 / 试题 / 答题 / AI 额度 / 管理员
 */
...
before(async () => {
  // 用与 server.js 完全相同的装配方式，保证测试和本地开发跑的是同一套东西
  const boot = await bootstrap({
    file: ':memory:',
    seed: false,
    readStatic: makeStaticReader(path.join(ROOT, 'public')),
    env: { LEARNHUB_SECRET: 'test-secret', LEARNHUB_DAILY_AI_LIMIT: '3' },
  });
  const { app, db } = boot;
  d1 = boot.d1;
